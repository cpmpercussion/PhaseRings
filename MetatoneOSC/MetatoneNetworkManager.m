...
#pragma mark TODO There really should be a "web classifier closed" callback to the delegate.
    id<MetatoneNetworkManagerDelegate> d = self.delegate;
    dispatch_async(dispatch_get_main_queue(), ^{
        [d stoppedSearchingForLoggingServer];
    });
}
...
-(void)webSocketDidOpen:(SRWebSocket *)webSocket {
    ...
    id<MetatoneNetworkManagerDelegate> d = self.delegate;
    NSString *host = self.webClassifierHostname;
    int port = self.webClassifierPort;
    dispatch_async(dispatch_get_main_queue(), ^{
        [d loggingServerFoundWithAddress:host andPort:port andHostname:host];
    });
}
...
-(void)netServiceDidResolveAddress:(NSNetService *)sender {
    ...
    if ([sender.type isEqualToString:OSCLOGGER_SERVICE_TYPE] && firstAddress && firstPort) {
        ...
        id<MetatoneNetworkManagerDelegate> d = self.delegate;
        NSString *addr = self.loggingIPAddress;
        int port = (int)self.loggingPort;
        NSString *host = self.loggingHostname;
        dispatch_async(dispatch_get_main_queue(), ^{
            [d loggingServerFoundWithAddress:addr andPort:port andHostname:host];
        });
        [self sendMessageOnline];
    ...
    if ([sender.type isEqualToString:METATONE_SERVICE_TYPE] && firstAddress && firstPort) {
        ...
        if (![firstAddress isEqualToString:self.localIPAddress] && ![firstAddress isEqualToString:@"127.0.0.1"]) {
            ...
            id<MetatoneNetworkManagerDelegate> d = self.delegate;
            NSString *addr = firstAddress;
            int port = firstPort;
            NSString *host = sender.hostName;
            dispatch_async(dispatch_get_main_queue(), ^{
                [d metatoneClientFoundWithAddress:addr andPort:port andHostname:host];
            });
...
/// All delegate dispatches are marshalled to the main queue — F53OSC and
/// SocketRocket both deliver here on main today, but this guarantees the
/// contract even if that changes (e.g. Bonjour resolves on a worker thread).
    ...
    if ([message.addressPattern isEqualToString:@"/metatone/app"]) {
        ...
        if ([args count] == 3) {
            if ([args[0] isKindOfClass:[NSString class]] &&
                [args[1] isKindOfClass:[NSString class]] &&
                [args[2] isKindOfClass:[NSString class]])
            {
                if (![((NSString *) args[0]) isEqualToString:self.deviceID]) { // ignore self metatone messages.
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [d didReceiveMetatoneMessageFrom:args[0] withName:args[1] andState:args[2]];
                    });
                }
            }
        }
        ...
        // Gesture Message
        if ([args count] == 2) dispatch_async(dispatch_get_main_queue(), ^{
            [d didReceiveGestureMessageFor:args[0] withClass:args[1]];
        });
    } else if ([message.addressPattern isEqualToString:@"/metatone/classifier/ensemble/state"]) {
        //Ensemble State
        if ([args count] == 3) dispatch_async(dispatch_get_main_queue(), ^{
            [d didReceiveEnsembleState:args[0] withSpread:args[1] withRatio:args[2]];
        });
    } else if ([message.addressPattern isEqualToString:@"/metatone/classifier/ensemble/event/new_idea"]) {
        if ([args count] == 2) dispatch_async(dispatch_get_main_queue(), ^{
            [d didReceiveEnsembleEvent:@"new_idea" forDevice:args[0] withMeasure:args[1]];
        });
    } else if ([message.addressPattern isEqualToString:@"/metatone/classifier/ensemble/event/solo"]) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [d didReceiveEnsembleEvent:@"solo" forDevice:args[0] withMeasure:args[1]];
        });
    } else if ([message.addressPattern isEqualToString:@"/metatone/classifier/ensemble/event/parts"]) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [d didReceiveEnsembleEvent:@"parts" forDevice:args[0] withMeasure:args[1]];
        });
    } else if ([message.addressPattern isEqualToString:@"/metatone/performance/start"]) {
        // performance start
        if ([args count] == 4) dispatch_async(dispatch_get_main_queue(), ^{
            [d didReceivePerformanceStartEvent:args[0]
                                     forDevice:args[1]
                                      withType:args[2]
                                andComposition:args[3]];
        });
    } else if ([message.addressPattern isEqualToString:@"/metatone/performance/end"]) {
        // performance stop
        if ([args count] == 2) dispatch_async(dispatch_get_main_queue(), ^{
            [d didReceivePerformanceEndEvent:args[0] forDevice:args[1]];
        });
        ...
        if ([args count] == 4) dispatch_async(dispatch_get_main_queue(), ^{
            [d didReceiveTouchPlayMessageFor:args[0] X:args[1] Y:args[2] vel:args[3]];
        });
    } else if ([message.addressPattern isEqualToString:@"/metatone/playback/gesture"]) {
        // Gesture Message to be played back!
        if ([args count] == 2) dispatch_async(dispatch_get_main_queue(), ^{
            [d didReceiveGesturePlayMessageFor:args[0] withClass:args[1]];
        });
    } else {
