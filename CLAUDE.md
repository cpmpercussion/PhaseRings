...
- **Build numbers**: "Use Xcode Cloud build number" is enabled on the workflow, which overrides `CFBundleVersion` on CI builds. Locally, the app and AUv3 appex both derive their versions from the project-wide `MARKETING_VERSION` (`3.0`) and `CURRENT_PROJECT_VERSION` (`200`) in `project.yml` — change them there, in one place. Build numbers are scoped per `CFBundleShortVersionString`, so the CI counter starts fresh from 1 for each marketing version — that's expected and fine. (PhaseRingsKit.framework stays at build `1` via XcodeGen's framework preset; embedded frameworks are exempt from the app/appex match requirement.)
